...
- **Configuration is mandatory**:
  - Both required for any run (main, tests):
    - `config/config.toml` — all search, platform, browser, and logging settings
...
- **Standard library TOML**: Python 3.11+ uses built-in `tomllib`; Python 3.10 uses `tomli` (included as conditional dependency)
...
- **Browser mode**: Controlled via `config.toml` (`browser.headless`), not code flags
- **Logging**: Configured in `config.toml` under `logging.file_path`; directory is auto-created
...
- `tomllib` import pattern: always use try/except for Python 3.10 compatibility (see `src/autoapply/main.py` for example)
